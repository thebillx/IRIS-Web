...
The machine default project and permission mode are persisted. Session identities/current-project, connected-client state, and pending owner approvals are intentionally transient. Pending approvals expire and are not durable authority. The Web App may retain only a selected-session ID in browser session storage as a reconnect preference; that pointer is never a second session store and is discarded when the authoritative daemon no longer lists the session.
