...
The machine default project and permission mode are persisted. Session identities/current-project, bounded interaction history, execution state, connected-client state, and pending owner approvals are intentionally transient. Pending approvals expire and are not durable authority. A browser refresh or reconnect to the same daemon reads that transient session history back from the authoritative runtime, but a daemon restart intentionally starts with no sessions or interaction history and never replays completed execution. The Web App may retain only a selected-session ID in browser session storage as a reconnect preference; that pointer is never a second session or conversation store and is discarded when the authoritative daemon no longer lists the session.

Production-provider credentials are not added to the runtime JSON storage model in this slice. OpenAI selection/model/credential are supplied through the explicit daemon child environment allowlist; the credential is not persisted by IRIS and is never copied into project registry, endpoint metadata, health, audit, or session history.
