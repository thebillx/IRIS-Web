...
const PROFILES: readonly ProfileDefinition[] = [
  ...
  { id: 'pnpm-script', executable: 'pnpm', candidates: () => [path.join(path.dirname(canonicalNodeRuntime().path), 'pnpm'),'/opt/homebrew/bin/pnpm','/usr/local/bin/pnpm'], minTimeoutMs: 100, maxTimeoutMs: 60 * 60_000, envAllowlist: ['CI','NODE_ENV','API_TOKEN','AUTH_TOKEN','PASSWORD'], effectEnvelope: CONSERVATIVE_EFFECTS, argvPolicy: 'PNPM_SCRIPT' },
  { id: 'npm-script', executable: 'npm', candidates: () => [path.join(path.dirname(canonicalNodeRuntime().path), 'npm'),'/opt/homebrew/bin/npm','/usr/local/bin/npm','/usr/bin/npm'], minTimeoutMs: 100, maxTimeoutMs: 60 * 60_000, envAllowlist: ['CI','NODE_ENV','API_TOKEN','AUTH_TOKEN','PASSWORD'], effectEnvelope: CONSERVATIVE_EFFECTS, argvPolicy: 'NPM_SCRIPT' },
