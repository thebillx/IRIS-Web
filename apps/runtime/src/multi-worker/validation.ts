...
function isWorker(value: unknown): value is Worker {
  return isRecord(value)
    ...
    && ['IDLE', 'ASSIGNED', 'STARTING', 'RUNNING', 'WAITING', 'SUCCEEDED', 'FAILED', 'CANCELLED', 'BLOCKED'].includes(String(value.state))
    && bounded(value.parentOrchestratorId, 200)
...
function isTask(value: unknown): value is WorkerTask {
  return isRecord(value)
    ...
    && ['PENDING', 'ASSIGNED', 'STARTING', 'RUNNING', 'WAITING', 'SUCCEEDED', 'FAILED', 'CANCELLED', 'BLOCKED'].includes(String(value.state))
