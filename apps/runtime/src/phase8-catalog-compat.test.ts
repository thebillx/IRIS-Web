...
const ADDITIVE_TOOLS = ['owner_approval_resolve', 'workspace', 'fs', 'artifact', 'shell', 'job', 'git', 'code_review'] as const;

describe('Phase 8 two-version catalog compatibility', () => {
  it('AC-IRIS-005 + AC-COMPAT-004 preserves the exact 42-tool 2.3.0 legacy schemas in the 2.5.0 catalog', () => {
    expect(PHASE8_V230_CATALOG_FIXTURE.catalogVersion).toBe('2.3.0');
    expect(MCP_CATALOG_VERSION).toBe('2.5.0');
