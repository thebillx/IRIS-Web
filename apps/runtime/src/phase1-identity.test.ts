...
describe('IRIS vNext Phase 1 identity and fencing', () => {
  ...
  it('AC-IRIS-006 preserves every legacy FULL tool, adds Phase 2/3/4 grouped tools, and keeps PRO at the exact read-only five', () => {
    ...
    expect(fullNames.filter((name) => !LEGACY_FULL_TOOL_NAMES.includes(name as (typeof LEGACY_FULL_TOOL_NAMES)[number]))).toEqual(['ado_discovery', 'ado_workitem_read', 'ado_hierarchy_read', 'ado_context_search', 'ado_backlog_list', 'owner_approval_resolve', 'workspace', 'fs', 'artifact', 'shell', 'job', 'git', 'code_review', 'security_audit']);
